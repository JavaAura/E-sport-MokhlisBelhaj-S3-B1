...
public class MenuJeu {
	...
	public void displayJeu() {
		...
		LoggerUtil.info(String.format("%-20s %-20s %-20s %-20s", "ID", "Nom", "Difficulté", "Durée moyenne"));
		LoggerUtil.info("--------------------------------------------------------------------------------");

		for (Jeu jeu : jeux) {
			LoggerUtil.info(String.format("%-20s %-20s %-20s %-20s", truncate(jeu.getId().toString(), 20),
					truncate(jeu.getNom(), 20), truncate(jeu.getDifficulte().toString(), 20),
